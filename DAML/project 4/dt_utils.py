...
def featureplot(d_train, t_train, n_classes,
    ...
    for pairidx, pair in enumerate(pairs):
        ...
        if (splitsample):
            pair_train = d_train.iloc[:, pair]
            pair_test = d_test.iloc[:, pair]
        else:
            pair_train = d_train.iloc[:, pair]
            pair_test = d_train.iloc[:, pair]
        ...
        for i, color in zip(range(n_classes), plt_colors):
            ...
            if (splitsample):
                ...
                idx = np.where(t_train == i)[0]

            if (c_names is not None):
                plt.scatter(pair_test.iloc[idx, 0], pair_test.iloc[idx, 1], c=color,
                            label=c_names[i], alpha=0.5,
                            #cmap=plt.cm.RdYlBu,
                            edgecolor='black', s=15)
            else:
                plt.scatter(pair_test.iloc[idx, 0], pair_test.iloc[idx, 1], c=color, alpha=0.5,
                            #cmap=plt.cm.RdYlBu,
                            edgecolor='black', s=15)
    ...
    if (t_names is not None):
        ## make legend larger ##
        plt.legend(loc='lower right', borderpad=0, handletextpad=0, fontsize=8)
    plt.axis("tight")
